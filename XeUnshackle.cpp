...
static float scrollX1 = 1280.0f;
static float scrollX2 = 1280.0f;
static const float scrollSpeed = 18.0f;
float scrollTitleY = -100.0f;
static bool hasStartedScrolling = false;

static const LPCWSTR scrollingText1 = L"Welcome to the Meme Edition build of XeUnshackle a version where it's just filled with retardation.";
static const LPCWSTR scrollingText2 = L"https://github.com/UnstoppableWolf/";
...
HRESULT XeUnshackle::Update()
{
    ...
    "VID9", "VID10", "VID11", "VID12", "VID13", "VID14", "VID15", "VID16", "VID17", "VID18", "VID19", "VID20", "VID21", "VID22", "VID23", "VID24", "VID25", "VID26", "VID27", "VID28", "VID29"
};
...
HRESULT XeUnshackle::Render()
{
    ...
    {
        ATG::RenderBackground(0xFF00FF00, 0xFF000032);
        m_Font.Begin();
        m_Font.SetScaleFactors(1.5f, 1.5f);

        // Vertical scroll logic for title header
        if (scrollTitleY < 0.0f)
        {
            scrollTitleY += scrollSpeed;
            if (scrollTitleY > 0.0f)
                scrollTitleY = 0.0f;
        }
        m_Font.DrawText(0, (int)scrollTitleY, YellowText, wTitleHeaderBuf);
        ...
        if (hasStartedScrolling)
        {
            ...
            m_Font.DrawText((int)scrollX1, 100, PurpleText, scrollingText1);
            m_Font.DrawText((int)scrollX2, 540, PurpleText, scrollingText2);
        }
        ...
        m_Font.DrawText(840, 530, WhiteText, currentLocalisation->MainScrBtnSaveInfo);
        m_Font.DrawText(840, 560, WhiteText, currentLocalisation->MainScrBtnDump1BL);
        m_Font.DrawText(840, 600, WhiteText, currentLocalisation->MainScrBtnExit);
        m_Font.End();
    }
...
VOID __cdecl main()
{
    ...
    if (!Hvx::CheckPPExpHVAccess())
    {
        if (!Hvx::DisableExpChecks())
        {
    ...
    // Part 2 - Apply kernel patches if needed
    cprintf("[XeUnshackle] Checking kernel patch state");
    if (*(DWORD*)0x80108E70 != 0x48003134)
    {
    ...
    // ============================================
    // === UI Display Strings Initialization ======
    // ============================================
    ...
    scrollX1 = 1280.0f; // copyright
    scrollX2 = 1280.0f; // "LOADING MODULES..."
    scrollTitleY = -100.0f; // XeUnshackle header scroll from off-screen top
